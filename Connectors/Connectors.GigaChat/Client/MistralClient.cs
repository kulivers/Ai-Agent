...
/// The Mistral client.
/// </summary>
internal sealed class GigaChatClient
{
    internal GigaChatClient(
        string modelId,
    ...
    internal async Task<IReadOnlyList<ChatMessageContent>> GetChatMessageContentsAsync(ChatHistory chatHistory, CancellationToken cancellationToken, PromptExecutionSettings? executionSettings = null, Kernel? kernel = null)
    {
        ...
        var mistralExecutionSettings = GigaChatAIPromptExecutionSettings.FromExecutionSettings(executionSettings);
...
#pragma warning restore CA1031
    ...
    private async IAsyncEnumerable<StreamingChatMessageContent> StreamChatMessageContentsAsync(ChatHistory chatHistory, GigaChatAIPromptExecutionSettings executionSettings, ChatCompletionRequest chatRequest, string modelId, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        this.ValidateChatHistory(chatHistory);
    ...
    private static readonly string s_namespace = typeof(GigaChatAIChatCompletionService).Namespace!;
    ...
    private ChatCompletionRequest CreateChatCompletionRequest(string modelId, bool stream, ChatHistory chatHistory, GigaChatAIPromptExecutionSettings executionSettings, Kernel? kernel = null)
    {
        if (this._logger.IsEnabled(LogLevel.Trace))
        {
    ...
    private Uri GetEndpoint(GigaChatAIPromptExecutionSettings executionSettings, string path)
    {
